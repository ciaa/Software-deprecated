...
namespace ciaa {
  namespace comms {
    namespace drivers {
      ...
      ciaaErrorCode ciaaCommQIODeviceAdapter::read(std::int32_t timeout,
                                                         char *data,
                                                         ciaa_size_t *n_bytes) {
      ...
      void ciaaCommQIODeviceAdapter::read(char *data,
        ...
        conexion_->connect(conexion_,
          ...
          qint64 readed = conexion_->read(data, *n_bytes);
          if (*n_bytes == readed) {
            callback(ciaaErrorCode::OK, readed);
          } else {
            callback(ciaaErrorCode::read_error, readed);
          }
        });
      ...
      ciaaErrorCode ciaaCommQIODeviceAdapter::write(std::int32_t timeout,
                                                          const char *data,
                                                          ciaa_size_t *n_bytes) {
      ...
      void ciaaCommQIODeviceAdapter::write(const char *data,
        ...
        QObject::connect(conexion_,
          ...
          QObject::disconnect(conexion_, &QTcpSocket::readyRead, nullptr, nullptr);
          callback(ciaaErrorCode::OK, bytes);
        });
